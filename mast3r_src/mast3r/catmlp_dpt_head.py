...
# @MODIFIED
def reg_desc_unnorm(desc, mode):
    if "norm" in mode:
        return desc  # Disable normalization by returning desc unchanged
    else:
        raise ValueError(f"Unknown desc mode {mode}")
    return desc
...
def postprocess(
    ...
    if desc_dim is not None:
        ...
        res["desc"] = reg_desc_unnorm(
            fmap[..., start : start + desc_dim], mode=desc_mode
        )  # @MODIFIED - Now returns un-normalized features
        if two_confs:
